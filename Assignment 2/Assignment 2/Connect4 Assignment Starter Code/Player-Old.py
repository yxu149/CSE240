...
class AIPlayer:
    ...
    def min_value(self, state, depth, alpha, beta):
        ...
        if depth == 0 or self.is_terminal_node(state, opponent) or self.is_terminal_node(state, player):
            return None, self.eval_frame(state, opponent, player)
    ...
    def max_value(self, state, depth, alpha, beta):
        ...
        if depth == 0 or self.is_terminal_node(state, opponent) or self.is_terminal_node(state, player):
            return None, self.eval_frame(state, player, opponent)
    ...
    def is_almost_winning(self, board, player):
        for c in range(COLUMN_MAX - 3):
            for r in range(ROW_MAX):
                if board[r][c] == player \
                        and board[r][c + 1] == player \
    ...
    def eval_frame(self, state, player, opponent):
        ...
        if self.is_winning_move(state, player):
            score += 10000
        elif self.is_almost_winning(state, player):
            score += 3000
        elif self.is_almost_winning(state, opponent):
            score += 1000
